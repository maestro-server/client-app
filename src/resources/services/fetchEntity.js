...
const FetcherData = Entity => (opts = {}, headers = {}) => {
  ...
  return {
    find(fn, query = {}) {
      ...
      const fk = `${k}_${queryRer}_${cache_key}`;

      CacheRequester(fk)(opts)(tenant)(fn).process(end => {
    ...
    create(fn, model, kcache = "list") {
      ...
      CacheRequester(fk)(opts)(tenant)(fn).remove(end => {
    ...
    patch(fn, model, gid) {
      ...
      CacheRequester(fk)(opts)(tenant)(fn).remove(end => {
        new Entity(model, path).authorization().patchID(key, end);
      });
    },
    ...
    remove(fn, model, gid) {
      ...
      CacheRequester(fk)(opts)(tenant)(fn).remove(end => {
        new Entity(model, path).authorization().deleteID(key, end);
      });

      CacheRequester(list_k)(opts)(tenant)(fn).remove(end => end);
    }
  };
};
