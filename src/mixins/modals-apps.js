...
export default {
  ...
  computed: {
    ...
    labelBtnChangeProvider() {
      return this.other
        ? "Back to selection " + this.labelPService
        : '<i class="fa fa-plus"></i> ' + this.labelPService;
    },
    changeType() {
  ...
  methods: {
    ...
    afterShow() {
      this.text.title = this.create
        ? `Create new ${this.family}`
        : `Edit ${this.model.name} ${this.family.toLowerCase()}`;
    },
    ...
    createSave() {
      ...
      FetchEntity(Applications)().create(this.finishJob, this.model);
    },

    editSave() {
      this.setupModel();

      FetchEntity(Applications)().update(this.finishJob, this.model);
    },
