...
export default {
  ...
  computed: {
    showASM() {
      return (
        this.data.storage_types === "ASM" || this.data.storage_types === "ACFS"
      );
    },
    findServices() {
  ...
  methods: {
    ...
    editLoad() {
      ...
      this.$set(
        this.data,
        "storage_types",
        _.get(this, "provider.storage_types")
      );
    ...
    setupModel() {
      const services = _.get(this.model, "services", []).map(
        this.findMatchesServices(this.service)
      );
    ...
    editSave() {
      ...
      FetchEntity(Servers)().patch(this.finishJob, this.model);
    },
    ...
    fetchServer(force = true) {
      if (_.has(this, "model._id")) {
        FetchEntity(Servers)({ force }).findOne(e => {
    ...
    setService(search) {
      if (!_.isEmpty(search)) {
        ...
        _.defaults(
          configs,
          _.pick(this.provider, ["storage_types", "dataguard"])
        );
