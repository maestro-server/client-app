  ...
  methods: {
    updateFilters(items) {
      if (items) {
        ...
        this.options.fields = this.options.filters.filters.map(
          e => `${e.key} (${e.type})`
        );
      }
    },
    ...
    setupObjectFilter(field) {
      this.options.subfield = this.getInternalField(field);
    },
    ...
    setupSelectFilter(field) {
      this.opts = this.getInternalField(field, "[0].opts");
